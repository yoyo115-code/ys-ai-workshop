...
应用启动时会执行幂等 schema，并删除已过期 Session。`database/migrations/0001_career_match.sql` 记录 Career Match 增量，`0002_resume_versioning.sql` 记录 Resume Optimizer 四表与索引。不会自动创建演示用户；只有明确配置初始化管理员时才会创建管理员。
